...
export function NodeEl({ node: n, state, onDragBegin, onSelect, editNodeId, onEditDone }: Props) {
  ...
  const handleDelete = useCallback((e: React.MouseEvent) => {
    ...
    const msg = childCount > 0 ? `Delete "${n.label}" and its ${childCount} child block(s)?` : `Delete "${n.label}"?`;
  ...
  const handleClick = useCallback((e: React.MouseEvent) => {
    ...
    if (tapTimer.current && tapId.current === n.id) {
      ...
      onSelect(n); // sets editNodeId via parent
      return;
    }
    tapId.current = n.id;
    tapTimer.current = window.setTimeout(() => {
      ...
      } else { setSel(null); setSelNodeId(null); }
    }, 270);
    ...
    >
      {isEditing ? (
        <input
          ...
          onKeyDown={e => {
            e.stopPropagation();
            if (e.key === 'Enter') { e.preventDefault(); inputRef.current?.removeEventListener('blur', commitEdit); commitEdit(); }
            if (e.key === 'Escape') { n.label = n.label; onEditDone(); }
          }}
        />
      ) : (
      ...
      {/* + handle: add child or connect/reparent */}
      {canConnect(n) && (
        <div
          className="nd-handle nd-handle-add"
          onMouseDown={e => { e.stopPropagation(); onDragBegin(n, e.currentTarget.parentElement as HTMLElement, e.clientX, e.clientY, 'connect'); }}
          onClick={e => e.stopPropagation()}
        >+</div>
      )}
