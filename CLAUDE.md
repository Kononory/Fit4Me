...
src/
  ...
  #               fetchPageStructure, fetchBatchThumbnails, parseFigmaFileKey
  #               SectionResult: { id, name, order, frames[] }
  #               PageResult now includes sections?: SectionResult[]
...
api/
  ...
  figma-page.ts    # file page structure: GET /api/figma-page?fileKey&token → { pages } (depth=3, includes sections[])
  figma-batch.ts   # batch thumbnails: GET /api/figma-batch?fileKey&nodeIds&token → { urls }
  figma-analyze.ts # Gemini IA grouping: POST /api/figma-analyze { frames[] } → { groups[{ name, frameIds }] }
```
...
- Color tokens (CSS custom properties in `:root`, warm oklch values): `--background` (warm white), `--card` (warm light), `--muted` (warm muted bg), `--accent` (hover bg), `--foreground` (near-black text), `--muted-foreground` (muted text, covers old #9A9995/#AEADA8), `--secondary-foreground` (secondary labels, old #5A5955), `--border` (warm border), `--input` (slightly lighter border), `--ring` (subtle/placeholder, old #BCBBB7), `--primary` / `--primary-foreground` (dark fill / white text), `--destructive` (red), `--teal` (shadcn teal-600, replaces green #6B9B5E), `--yellow` (shadcn yellow-500, replaces orange #C8963C)
- Semantic one-off colors kept as raw hex (not tokenized): `#FCECEA`/`#FDECEA`/`#FFF0EF` (error tint bgs), `#FDF4E7`/`#FEF3E2` (warning tint bgs), `#E8F2E6` (success tint bg), `#4A90E2` (Figma/link blue), `#EEEAFF`/`#9B8FD4` (edge type purple), `#DEB87A`/`#D98A83` (border tints)
...
- No FigmaImportModal.tsx read (~4 k) — step machine + importMode pattern now listed in repo layout above
...
- Translation: DeepL Free API primary, Gemini 2.0 Flash Lite fallback on 456/429; env vars `Fit4Me_DEEPL_API_KEY` + `Fit4Me_GEMINI_API_KEY`; ZH→ZH-HANS, PT→PT-BR; HE routed to Gemini (DeepL doesn't support it)
